...
char * getTemperatureReading(){
  ...
  dtostrf(temperatureC, 0, 4, buffer);
...
void postDeviceState(){
  uint16_t statuscode;
  int16_t length;
  char url[] = "http://198.199.84.221:3001/api/firefighter/state";
  char data[200];

  // flushSerial();
  Serial.println(F("Sending Device State"));
  ...
  char * temp = getTemperatureReading();
  char * gasLevel = getGasReading();

  // char gas_reading[]  = "123";
  // char temp_reading[] = "123";
  char ccid[] = "8901260966194108872f"; //fona.getSIMCCID(replybuffer); 
  char status[] = "true";

  sprintf(data,"{\"ccid\":\"%s\", \"status\":\"%s\", \"val_gas_reading\":\"%s\", \"val_temp_reading\":\"%s\"}", ccid, status, gasLevel, temp);
